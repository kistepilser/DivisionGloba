...
var STATES={
  ...
  moved:{label:'Переведён на другой объект',txt:'→',cls:'s-moved'}
};

/* ---------- Должности ---------- */
var POSITIONS=['Продавец','Старший продавец','Кассир','Товаровед','Управляющий','Заместитель управляющего','Администратор','Кладовщик','Грузчик','Уборщица','Охранник'];
...
/* ---------- Переводы между объектами дивизиона ---------- */
/* e.transfers = [{objId:'o2',from:'2026-08-01',graph:5}] — постоянный перевод с указанной даты */
function transfersOf(e){
  var t=(e&&e.transfers)||[];
  return t.filter(function(x){return x&&x.objId&&parseDate(x.from);})
          .slice().sort(function(a,b){return parseDate(a.from)-parseDate(b.from);});
}
/* на каком объекте человек числится в этот день */
function objectOn(e,y,m,d){
  if(!e)return null;
  var n=dnum(y,m,d),cur=e.objectId,list=transfersOf(e);
  for(var i=0;i<list.length;i++){if(n>=parseDate(list[i].from))cur=list[i].objId;}
  return cur;
}
function graphOn(e,y,m,d){
  var n=dnum(y,m,d),g=e?e.graph:0,list=transfersOf(e);
  for(var i=0;i<list.length;i++){if(n>=parseDate(list[i].from)&&Number(list[i].graph))g=list[i].graph;}
  return g;
}
function currentObjectId(e){
  var t=new Date();
  return objectOn(e,t.getFullYear(),t.getMonth()+1,t.getDate());
}
function worksAtOn(e,objId,y,m,d){return employedOn(e,y,m,d)&&objectOn(e,y,m,d)===objId;}
function worksAtInMonth(e,objId,y,m){
  if(!employedInMonth(e,y,m))return false;
  if(!transfersOf(e).length)return e.objectId===objId;
  var dim=daysInMonth(y,m);
  for(var d=1;d<=dim;d++)if(worksAtOn(e,objId,y,m,d))return true;
  return false;
}
function transferTags(e){
  return transfersOf(e).map(function(t){
    var o=objById(t.objId);
    return '<span class="tag violet">→ '+esc((o&&o.code)||'?')+' с '+esc(fmtDate(t.from))+'</span> ';
  }).join('');
}
function transferText(e){
  var l=transfersOf(e);
  if(!l.length)return '';
  return l.map(function(t){var o=objById(t.objId);return 'перевод в '+((o&&o.name)||'?')+' с '+fmtDate(t.from);}).join(' · ');
}
...
function empHome(e){
  ...
  var o=objById(currentObjectId(e));
...
function autoVacancies(objId,y,m){
  ...
  db.employees.filter(function(e){
    if(e.external||e.vacancy)return false;
    return y?worksAtInMonth(e,objId,y,m):currentObjectId(e)===objId;
  }).forEach(function(e){
...
function empsFor(objId,y,m){
  var own=db.employees.filter(function(e){
    if(e.external)return false;
    if(e.vacancy)return e.objectId===objId;
    return y?worksAtInMonth(e,objId,y,m):currentObjectId(e)===objId;
  });
...
function cellState(emp,y,m,d,objId){
  var mk=db.marks[key(emp.id,y,m,d)];
  var base=isShift(graphOn(emp,y,m,d),y,m,d);
  if(!emp.vacancy&&!employedOn(emp,y,m,d))return {type:'none',base:base,out:true};
  if(objId&&!emp.vacancy&&!isExternal(emp)&&objectOn(emp,y,m,d)!==objId)
    return {type:'moved',base:base,objId:objectOn(emp,y,m,d)};
  if(mk){
...
function needsCover(emp,st){
  ...
  if(st.type==='moved')return st.base;
...
function guestsFor(objId,y,m){
  ...
  db.employees.filter(function(e){return currentObjectId(e)!==objId&&!e.vacancy;}).forEach(function(e){
    var days=[];
...
function candidatesFor(objId,y,m,d){
  var free=[],busy=[];
  db.employees.filter(function(e){return !e.vacancy&&objectOn(e,y,m,d)!==objId&&e.fio;}).forEach(function(e){
