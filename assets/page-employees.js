...
function renderEmps(){
  ...
  var cards=objs.map(function(o){
    var staff=db.employees.filter(function(e){return currentObjectId(e)===o.id&&!e.external&&!e.vacancy;});
    ...
    var rows=shown.length?shown.map(function(e,i){
      return '<tr'+(e.firedAt?' class="fired"':'')+'><td class="muted">'+(i+1)+'</td>'+
        ...
        '<td class="dates">'+(empPeriodTags(e)+transferTags(e)||'<span class="muted">бессрочно</span>')+'</td>'+
        '<td class="right">'+(isAdmin()
          ?'<button class="btn ghost sm" onclick="openEmpModal(&#39;'+e.id+'&#39;)">Изменить</button> '+
           '<button class="btn ghost sm" onclick="openTransferModal(&#39;'+e.id+'&#39;)">Перевод</button> '+
           '<button class="btn sm '+(e.firedAt?'ok':'warn')+'" onclick="openFireModal(&#39;'+e.id+'&#39;)">Увольнение</button> '+
  ...
  var toolbar='<div class="toolbar">'+
    ...
    '<div class="row"><button class="btn sm" onclick="openAllObjs(true)">Раскрыть всё</button>'+
      ...
      (isAdmin()?'<button class="btn sm" onclick="openTransferModal()">⇄ Перевод</button>'+
        '<button class="btn warn sm" onclick="openFireModal()">Увольнение</button>'+
...
/* ---------- перевод на другой объект дивизиона ---------- */
function openTransferModal(id){
  if(!needAdmin())return;
  var staff=db.employees.filter(function(e){return !e.vacancy&&!e.external&&e.fio;});
  if(!staff.length){toast('Сначала добавьте сотрудников');return;}
  if(db.objects.length<2){toast('Для перевода нужно минимум два объекта');return;}
  var cur=(id?empById(id):null)||staff[0];
  var t=new Date(),iso=t.getFullYear()+'-'+String(t.getMonth()+1).padStart(2,'0')+'-'+String(t.getDate()).padStart(2,'0');
  var from=objById(currentObjectId(cur));
  modal('Перевод на другой объект',
    '<div class="field"><label class="f">Сотрудник</label><select id="t-emp" onchange="syncTransfer()">'+
      staff.map(function(e){
        var h=empHome(e);
        return '<option value="'+e.id+'"'+(e.id===cur.id?' selected':'')+'>'+esc(e.fio)+' — '+esc(h.code)+
          (e.position?' · '+esc(e.position):'')+'</option>';
      }).join('')+'</select></div>'+
    '<div class="field"><label class="f">Сейчас числится</label>'+
      '<div id="t-now" class="muted" style="font-size:13px">'+esc(from?from.name:'без объекта')+'</div></div>'+
    '<div class="field"><label class="f">Перевести на объект</label><select id="t-obj">'+
      db.objects.map(function(o){
        return '<option value="'+o.id+'"'+(from&&o.id!==from.id&&db.objects.indexOf(o)===0?'':'')+'>'+esc(o.name)+' — '+esc(o.code)+'</option>';
      }).join('')+'</select></div>'+
    '<div class="grid2"><div><label class="f">Работает там с (первая смена)</label>'+
      '<input id="t-date" type="date" value="'+esc(iso)+'"></div>'+
      '<div><label class="f">№ графика на новом месте</label>'+
      '<input id="t-graph" type="number" min="1" max="14" value="'+esc(cur.graph||1)+'"></div></div>'+
    '<p class="muted" style="font-size:12.5px">Перевод постоянный: до этой даты человек остаётся в графике старого объекта, а с неё — нового. '+
      'На старом месте его смены после перевода помечаются как незакрытые и ставка становится вакансией. Это не замена на день — для разовых выходов есть замены в графике.</p>'+
    '<div id="t-hist">'+transferHistHtml(cur)+'</div>',
    '<button class="btn danger" onclick="cancelTransfer()">Отменить последний перевод</button>'+
    '<button class="btn primary" onclick="applyTransfer()">Перевести</button>');
  syncTransfer();
}
function transferHistHtml(e){
  var l=transfersOf(e);
  if(!l.length)return '<p class="muted" style="font-size:12.5px">Переводов пока нет.</p>';
  return '<p class="muted" style="font-size:12.5px">История переводов: '+
    l.map(function(t){var o=objById(t.objId);return esc(((o&&o.name)||'?')+' — с '+fmtDate(t.from)+(Number(t.graph)?' (график №'+t.graph+')':''));}).join('; ')+'</p>';
}
function syncTransfer(){
  var e=empById(document.getElementById('t-emp').value);if(!e)return;
  var now=objById(currentObjectId(e));
  var n=document.getElementById('t-now');if(n)n.textContent=now?now.name:'без объекта';
  var h=document.getElementById('t-hist');if(h)h.innerHTML=transferHistHtml(e);
  var g=document.getElementById('t-graph');if(g)g.value=e.graph||1;
  var sel=document.getElementById('t-obj');
  if(sel&&now){
    for(var i=0;i<sel.options.length;i++){if(sel.options[i].value!==now.id){sel.selectedIndex=i;break;}}
    sel.dispatchEvent(new Event('change',{bubbles:true}));
  }
}
function applyTransfer(){
  var e=empById(document.getElementById('t-emp').value);if(!e)return;
  var objId=document.getElementById('t-obj').value;
  var d=document.getElementById('t-date').value;
  var g=Math.min(14,Math.max(1,Number(document.getElementById('t-graph').value)||Number(e.graph)||1));
  if(!d){toast('Укажите дату первой смены на новом объекте');return;}
  if(e.hiredAt&&d<e.hiredAt){toast('Дата раньше даты приёма');return;}
  if(e.firedAt&&d>e.firedAt){toast('Дата позже увольнения');return;}
  if(objId===currentObjectId(e)&&!transfersOf(e).length&&objId===e.objectId){toast('Это тот же объект');return;}
  e.transfers=(e.transfers||[]).filter(function(t){return t.from!==d;});
  e.transfers.push({objId:objId,from:d,graph:g});
  openObjs[objId]=true;saveOpen();
  var o=objById(objId);
  save();closeModal();render();toast(e.fio+' — перевод в '+((o&&o.code)||'')+' с '+fmtDate(d));
}
function cancelTransfer(){
  var e=empById(document.getElementById('t-emp').value);if(!e)return;
  var l=transfersOf(e);
  if(!l.length){toast('У этого сотрудника нет переводов');return;}
  var last=l[l.length-1];
  e.transfers=(e.transfers||[]).filter(function(t){return !(t.from===last.from&&t.objId===last.objId);});
  save();closeModal();render();toast(e.fio+' — последний перевод отменён');
}
