...
function renderSched(){
  ...
  emps.forEach(function(e,i){
    ...
    for(var d=1;d<=dim;d++){
      var st=cellState(e,y,m,d,obj.id),wd=new Date(y,m-1,d).getDay(),wknd=(wd===0||wd===6);
      var cls='cell',txt='';
      ...
      if(st.type==='moved'){var mo=objById(st.objId);cls='cell'+(st.base?' s-moved':'');txt=st.base?('→'+((mo&&mo.code)||'')):'';}
      ...
      if(st.type==='moved'){var mo3=objById(st.objId);title='Переведён в '+((mo3&&mo3.name)||'другой объект')+(st.base?' — смену здесь нужно перекрыть':'');}
    ...
    rows+='<tr class="'+(e.vacancy?'vacancy':'')+'">'+
      ...
      '<td class="c-fio sticky2">'+esc(empName(e))+
        ...
        '<span class="pos">'+esc(e.position||'')+(cover?' · перекрыть: '+cover:'')+(transferText(e)?' · '+esc(transferText(e)):'')+'</span></td>'+
...
function openCell(empId,d){
  ...
  var st=cellState(e,ui.year,ui.month,d,ui.objectId);
